...
/// CLI's login file and per-project folders, Antigravity's conversation folders, Copilot's config folder, and Kimi
/// Code's newest sessions.
struct AgentActivity: Sendable {
    ...
    /// Gemini CLI rewrites its login file on every token refresh and keeps a folder per project under `tmp`. Each
    /// folder is one listing, not a walk.
