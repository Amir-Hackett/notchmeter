...
extension ToolID {
    ...
    /// The same colour as a pair of values, the dark one the rings draw on the black panel and the light one Paper
    /// prints it in (`PanelInk.tool`), for the surfaces that choose by hand rather than through a look: the
    /// Dashboard's charts (`chartColor`) and the review renders. NewRowsPresentation pins both against their surfaces.
    var identity: (light: UInt32, dark: UInt32) {
        (light: PanelInk.tool(self).onPaper.hex, dark: PanelInk.tool(self).onBlack.hex)
    }
