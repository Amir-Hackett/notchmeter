...
enum SessionDetection {
    ...
    /// The executable names each assistant ships under. Gemini CLI has its own row since 0.9.0 (the Antigravity IDE
    /// runs no process the scan could name). Codex's Homebrew cask runs its release binary under its own long name.
    /// Kimi Code and OpenCode are not scanned for: OpenCode's sessions come from its own database without a hook
    /// (OpenCodeSessions), and Kimi Code's process carries nothing the scan could read a project or a turn from.
