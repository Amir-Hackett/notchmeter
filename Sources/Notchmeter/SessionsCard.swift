...
struct SessionsCard: View {
    ...
    static func rows(_ sessions: [AgentSession], hideTitles: Bool, jump: Bool, now: Date) -> (rows: [Row], more: Int) {
        ...
        let rows = ordered.prefix(rowCap).map { session -> Row in
            ...
            var row = Row(id: session.id, tool: session.tool, title: lines.title, chips: [session.tool.displayName],
    ...
    /// counts they carry, never a title, a task's text, a teammate's name or an MCP server's (docs/testing.md).
