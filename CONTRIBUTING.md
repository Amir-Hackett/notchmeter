...
- A new page under `site/` names its canonical URL and goes into `site/sitemap.xml`; a new guide also goes into `site/guides/index.html`, carries the date and the *Tested on Notchmeter* stamp, and loads no script. `ReleasePackagingTests` (`SiteGuides`) checks all of it. A guide states nothing the app and its documents cannot back: when a rule changes, the guide that quotes it changes in the same pull request.
