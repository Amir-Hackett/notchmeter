...
- **The hook socket.** The commands reach the app over `~/Library/Application Support/Notchmeter/hook.sock`, and the app takes a line only from a process whose code satisfies its own designated requirement (docs/hooks.md, *How it reaches the app*). A process that is not a copy of Notchmeter getting a hook or a status line accepted, or reading one another process sent, is a security issue; so is the socket or its folder being created with permissions wider than `0600` and `0700`. Until 0.6.0 this hop was a distributed notification any process of the same user could read or forge.
