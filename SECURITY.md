...
- **The hook socket.** The commands reach the app over `~/Library/Application Support/Notchmeter/hook.sock`, and the app takes a line only from a process whose code satisfies its own designated requirement (docs/hooks.md, *How it reaches the app*). That proves the peer is a copy of Notchmeter, not who launched it: `Notchmeter --hook` forwards its standard input, so code of your own user that can run it is not stopped, and is covered by the out-of-scope line below; so is a same-user process that replaces the socket file and takes the lines, which silences the hooks and nothing more. What is a security issue: another process observing a line a hook sent to the app; a process that is not a copy of Notchmeter (another user, an App-Sandboxed app, a build with a different signature) getting a hook or a status line accepted; the socket or its folder being created with permissions wider than `0600` and `0700`. Until 0.6.0 this hop was a distributed notification any process of the same user could read.
...
Out of scope: the vendors' own services and terms; a Mac already compromised at the user level (the tokens are readable there by design, that is where the tools keep them, and the hook socket's check names the binary rather than the user's other code, which can run it); denial of service against the loopback API or the hook socket by a local process.
