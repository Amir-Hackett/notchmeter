...
@Suite struct SessionsCardRows {
    ...
    @Test func eachRowCarriesItsChipsClockStatusAndNote() {
        ...
        let (rows, more) = SessionsCard.rows([working, waiting, finished, remote], hideTitles: false, jump: true, now: now)
        #expect(more == 0)
        #expect(rows.map(\.status) == [.working, .waiting, .finished, .working])
        #expect(rows[0].chips == ["Claude", "iTerm"])
        #expect(rows[0].title == "Working on it")
        #expect(rows[0].since == t0.addingTimeInterval(10), "the clock is the turn's start when there is one")
        #expect(rows[0].note == nil)
        #expect(rows[0].canJump)
        #expect(rows[1].chips == ["Codex", "iTerm"])
        #expect(rows[1].note == .waitingForAnswer)
        #expect(rows[2].note == .doneJump)
        #expect(rows[2].since == t0, "no turn, so the session's own start")
        #expect(rows[3].chips == ["Claude", "@devbox"])
        #expect(rows[3].canJump == false, "a session on another Mac has nothing to jump to")
