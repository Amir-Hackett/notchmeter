...
@Suite struct FeedbackRedactionRules {
    ...
    static let names = FeedbackRedaction(home: home, values: [
    ...
    @Test func noProjectBranchTitlePathOrNameSurvives() {
        let text = """
            ...
            """
        let result = Self.names.apply(text)
...
@MainActor @Suite struct FeedbackRedactionGathering {
    ...
    /// home folder are all gone, and the report still reads as a report.
    @Test func theFixtureReportComesOutWithNoNameInIt() {
        let (store, prefs) = DemoFixtures.store(now: Date())
        let names = FeedbackRedaction.gather(store: store, prefs: prefs, home: DemoFixtures.home, accounts: [DemoFixtures.account])
        let report = names.apply(DemoFixtures.diagnostics(now: Date(), extraLines: 5))
        let message = names.apply(DemoFixtures.feedbackMessage)
        for text in [report.text, message.text] {
        ...
        #expect(report.text.contains("\(AppInfo.name) \(AppInfo.version) diagnostics"))
        #expect(message.home)
        let replaced = Feedback.Replaced([report, message])
...
@Suite struct FeedbackPayloadRules {
    ...
    static func report(lines: Int) -> Feedback.Report {
        ...
        return Feedback.Report(text: Diagnostics.report(facts, log: log, now: Date(timeIntervalSince1970: 0), home: "/Users/nobody"))
    }
    ...
    @Test func aShortPayloadIsSentWhole() throws {
        ...
        #expect(length <= Feedback.urlLimit)
    ...
    @Test func theOldestLogLinesGoFirstAndAsFewAsItTakes() throws {
        ...
        for route in [Feedback.Route.browser, .mailto] {
            ...
            let length = try #require(payload.url).absoluteString.utf8.count
            #expect(length <= Feedback.urlLimit)
            ...
            let more = Feedback.body(message: "Too long", about: Self.about, report: report, keep: kept + 1)
            let longer = try #require(Feedback.url(route: route, title: payload.title, body: more)).absoluteString.utf8.count
            #expect(longer > Feedback.urlLimit)
        }
    }

    @Test func whenNotEvenTheFactsFitTheDiagnosticsGoThenTheMessageIsCut() throws {
        let report = Self.report(lines: 10)
        let small = Feedback.payload(message: "Short", about: Self.about, report: report, route: .browser, limit: 400)
        #expect(small.cut == .diagnostics)
        #expect(small.body.hasSuffix("[Diagnostics left out to fit the length a link can carry]"))
        let smallLength = try #require(small.url).absoluteString.utf8.count
        #expect(smallLength <= 400)
        ...
        let cutLength = try #require(cut.url).absoluteString.utf8.count
        #expect(cutLength <= Feedback.urlLimit)
    }
...
@Suite struct FeedbackLinks {
    ...
    @Test func anIssueLinkCarriesTheTextExactly() throws {
        let url = try #require(Feedback.issueURL(title: "A + B", body: Self.awkward))
        #expect(url.host == "github.com")
        #expect(url.path == "/\(Feedback.repository)/issues/new")
        let fields = try query(url)
        #expect(fields["title"] == "A + B")
        #expect(fields["body"] == Self.awkward)
...
@Suite struct FeedbackRouting {
    ...
    @Test func theOracleHearsCountsAndNeverTheText() throws {
        ...
        let fields = Feedback.oracleFields(action: "sent", destination: .github, includesDiagnostics: false, payload: payload,
                                           replaced: Feedback.Replaced([message]))
