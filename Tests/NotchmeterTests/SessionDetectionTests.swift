...
@Suite struct SessionDetectionMatching {
    ...
    @Test func eachAssistantIsKnownByItsOwnNameOrItsScript() {
        ...
        #expect(SessionDetection.tool(command: "node", arguments: ["node", "/opt/homebrew/bin/gemini"]) == .gemini, "Gemini CLI has its own row since 0.9.0")
        #expect(SessionDetection.tool(command: "node", arguments: ["/opt/homebrew/bin/node", "/x/node_modules/@google/gemini-cli/dist/index.js"]) == .gemini)
...
@Suite struct SessionDetectionPlanning {
    ...
    @Test func aChainOfOneAssistantsProcessesIsOneRowAtItsInnermost() {
        let matched: [(process: SessionDetection.Process, tool: ToolID)] = [
            ...
            (process(30, "node", ["node", "/opt/homebrew/bin/gemini"]), .gemini),
            (process(31, parent: 30, "node", ["node", "/opt/homebrew/bin/gemini"]), .gemini),
