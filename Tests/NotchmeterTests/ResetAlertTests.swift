...
@Suite struct AdvisorExtras {
    ...
    ///
    /// The stopped session's exemption is now capped at one banner per session per ten minutes, because nothing
    /// bounded it: the app is never told a prompt was answered, so a repo with no allow rules would banner over
    /// the terminal being worked in as often as it prompts. These assertions pass no `lastBlockingBanner`, which
    /// is the no-allowance case — the first blocking wait, and the rule exactly as it read before the ceiling.
    /// The ceiling itself is pinned in `WaitBannerCeiling` (NotifierCopyTests.swift).
