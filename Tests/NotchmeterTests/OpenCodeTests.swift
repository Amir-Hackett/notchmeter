...
/// A tool a later version adds is switched on once for an install that saved its lists before it existed.
@Suite struct ToolIntroductionTests {
    @Test func anInstallFromBeforeOpenCodeIsOfferedIt() {
        #expect(ToolIntroduction.new(offered: nil) == [.opencode])
        #expect(ToolIntroduction.new(offered: ToolID.allCases.map(\.rawValue)).isEmpty)
    }
    ...
    @MainActor @Test func itIsSwitchedOnOnceAndStaysOffWhenSwitchedOff() {
        ...
        #expect(prefs.enabledTools == [.claude, .codex, .opencode])
        #expect(prefs.costCardTools == [.claude, .opencode])
        #expect(defaults.stringArray(forKey: "enabledTools") == ["claude", "codex", "opencode"])
