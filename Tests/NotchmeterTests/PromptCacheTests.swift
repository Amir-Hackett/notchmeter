...
@Suite struct PromptCacheDiagnostic {
    ...
    /// The session's name is shown under the same setting as the prompt title: with *Show what a session is
    /// working on* off the store drops it before the tracker, exactly as `hookReceived` drops `title`; and a row's
    /// title prefers the prompt's first line, falling back to the name only when no prompt has been sent yet.
    @MainActor @Test func theSessionNameFollowsTheTitleSetting() throws {
        let suite = "NotchmeterTests.promptCache.titles"
        let defaults = UserDefaults(suiteName: suite)!
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
        ...
        let store = UsageStore(prefs: prefs, providers: [], cache: ReadingCache(defaults: defaults), defaults: defaults,
                               drainLog: nil, reportFile: nil)
        var message = Statusline.Message(sessionID: "s3", project: "proj", model: "Opus", receivedAt: t0)
        message.sessionName = "a private title"
        message.linesAdded = 3
        store.statuslineReceived(message, now: t0)
        let session = try #require(store.sessions.all.first)
        #expect(session.sessionName == nil)
        #expect(session.linesAdded == 3, "only the name is a title; the figures still land")
        #expect(session.displayTitle == nil)

        var tracker = SessionTracker()
        tracker.statusline(sessionID: "s4", project: "proj", sessionName: "fix the notch", now: t0)
        #expect(tracker.all.first?.displayTitle == "fix the notch")
        var titled = try #require(tracker.all.first)
        titled.title = "Make the rings breathe"
        #expect(titled.displayTitle == "Make the rings breathe")
        titled.title = ""
        #expect(titled.displayTitle == "fix the notch", "an empty title is no title")
    }
