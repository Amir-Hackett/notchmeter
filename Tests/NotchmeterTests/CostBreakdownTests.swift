...
@Suite(.serialized) struct CostBreakdown {
    ...
    @Test func theCacheHoldsEntriesOnlyForRecentFilesAndClearsOlderVersions() async throws {
        ...
        let currentCacheName = "claude-usage-cache-v6.json"
        #expect(cacheName == currentCacheName, "0.8.0 changed the dedupe key and 0.9.0 added the price source to every bucket, so the version moved on twice and older digests re-parse")
