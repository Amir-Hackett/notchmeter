...
- A permission request or a multiple-choice question from Claude Code (and a permission request from Codex or Copilot CLI) opens in the panel: **Allow** (⌘Y), **Deny** (⌘N), or the option you pick (⌘1…⌘9), and *Answer in the terminal* to hand it back. A request nobody answers falls back to the terminal's own prompt, as if the notch were not there.
