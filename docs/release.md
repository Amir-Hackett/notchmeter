...
6. The tap follows on its own: `Amir-Hackett/homebrew-tap` runs `bump-notchmeter.yml` every 30 minutes, reads the latest promoted release, downloads its DMG and points `Casks/notchmeter.rb` at its version and sha256; a prerelease never reaches it, because `releases/latest` never resolves to one. Start it at once with `gh workflow run bump-notchmeter.yml -R Amir-Hackett/homebrew-tap` and watch that run to its end (`gh run list -R Amir-Hackett/homebrew-tap --workflow=bump-notchmeter.yml --event workflow_dispatch --limit 1` names it; `gh run watch <id> -R Amir-Hackett/homebrew-tap --exit-status` fails if it does), and give `packaging/homebrew/notchmeter.rb` the same version and sha256 (the script prints the sum; on path a it is in the job summary). Then `brew update && brew upgrade --cask notchmeter` and check `brew list --cask --versions notchmeter` names the new version: an upgrade that exits cleanly proves nothing on its own, which is how this Mac stayed on 0.7.8 through the 0.7.9 release. The tap is what users install from, not this repository's copy.
7. Add the release's row to `docs/release-state.md` in the private `Amir-Hackett/notchmeter-internal` repository: the tag's commit, the date promoted, whether it was launched from /Applications before promotion, the tap commit, and what went wrong.
