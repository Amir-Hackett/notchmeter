...
The even-burn projection and the last-hour drain both answer with one time. The interval answers with two, from the drain log: every hour of the last seven days in which the window moved gives a rate (used fraction per hour, from the log's rows, reset boundaries excluded); the 20th and 80th percentiles of those rates give the latest and the earliest run-out for what is left. How the two are shown is one rule, `RunOutInterval.presentation`, for the card and the advice (and the notification, which carries the advice line) alike: a range ("Runs out 2:10 PM–4:40 PM" on the card, "between 2:10 PM and 4:40 PM" in the advice) when the edges are more than fifteen minutes apart, one time at their midpoint when they are not, and "from 2:10 PM, or lasts to the reset" when only the fast edge falls before the reset. Until 0.6.0 the card took the midpoint and the advice the earliest edge, so the panel named two times for one event. Notifications fire on the earliest edge, which is a threshold rather than a display. Fewer than four rates give no interval, a run-out past the reset gives none, and with peak hours on and at least four rates on each side the estimate uses only the rates that match the current side of the boundary. It is a description of your own recent hours, not a model of the vendor's metering. Pinned by `RunOutTests`.
