...
| Assets | `docs/media`: GIF, seven PNGs (an Increase Contrast variant, the notch on each side edge, and the two signal states side by side); `docs/launch/gallery`: the eight Product Hunt frames — the hover as a GIF, seven stills — and the thumbnail, all drawn from fixture readings and fixture hook events by `--render-assets` and `--render-gallery` |
...
3. Send [anthropic-inquiry.md](anthropic-inquiry.md) in the first week of this window, not later; its cadence lines now say five minutes and mention the status line.
