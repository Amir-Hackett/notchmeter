...
**Days 1–7, shippable at all.** MIT licence: done. Public repository: done, 2026-09-05. README GIF: done (`docs/media/demo.gif`, rendered from fixtures). `scripts/test.sh` running in automation: done, on every pull request and on every tag. Colour accessibility and `monospacedDigit`: done. Swift 6 `Sendable` warnings: resolved, and zero warnings under `Sources/` is a gate on every tag.
