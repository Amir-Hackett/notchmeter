...
| Release pipeline | `scripts/release.sh` and `.github/workflows/release.yml`: universal binary, Developer ID, hardened runtime, notarisation, stapled DMG, EdDSA-signed Sparkle appcast, Homebrew cask file, gated on the zero-warnings release build and the tests. v0.1.0 shipped signed and notarised on 2026-09-05, **by hand**: `release.yml` was disabled with the other workflows and has never run. Enabled 2026-09-05; the next `v*` tag is its first real run, and it needs macOS minutes to exist on the account (Settings › Billing › Actions), or it fails before checkout like CI did on 2026-09-03 |
...
| Signed, notarised, Sparkle-updated release v0.1.0 and everything gated on it (tap, Setapp, Show HN, Product Hunt) | Done 2026-09-05: v0.1.0 is Developer ID signed, notarised and stapled, `scripts/Info.plist` carries the real `SUPublicEDKey`, the cask carries the released DMG's sha256 and the tap works. Setapp, Show HN and Product Hunt remain open. The GitHub Actions signing secrets are not set, so the workflow cannot publish a signed build until they are (docs/release.md, step 5; the Secrets workflow reports it on every push) | [release.md](release.md); `scripts/Info.plist`; `packaging/homebrew/notchmeter.rb` |
