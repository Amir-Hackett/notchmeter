...
| Incremental cost summing (per-file quarter-hour digests; entries read only inside the current block) | shipped | `FileDigest`; README "Energy" (figures not yet re-measured) |
...
| iCloud sync of usage history across Macs | Blocked from this machine: a CloudKit/iCloud container needs the `com.apple.developer.icloud-*` entitlements, which require Apple Developer Program enrolment and a provisioning profile; an ad-hoc-signed SwiftPM build cannot carry them. The daily-history file is laid out so a later sync only has to ship files | https://github.com/robinebers/openusage/blob/main/docs/icloud-sync.md |
| iPhone / Apple Watch companion (Live Activity, Dynamic Island, Lock Screen); macOS desktop widget | Blocked from this machine: an iOS/watchOS target, Live Activities and a WidgetKit extension need Xcode and an Apple Developer account to build, sign and install; only the Mac-side half (a Bonjour service publishing the `--probe --json` document) could be written here | https://github.com/wxtsky/CodeIsland ; https://github.com/robinebers/openusage ; https://github.com/robinebers/openusage/issues/968 ; https://github.com/robinebers/openusage/issues/422 |
...
| Public repository, GitHub topics, the Anthropic inquiry, launch posts, awesome-list submissions and the domain | The user's actions, all with copy written: flip the repo to public and apply the topics ([launch/awesome-lists.md](launch/awesome-lists.md)), send the Anthropic inquiry (kept in the private internal repository) from a personal address, post [show-hn.md](launch/show-hn.md) then [product-hunt.md](launch/product-hunt.md), file the list submissions on their gates, confirm notchmeter.app/.com at a registrar | this file, "Days 46–90" and "Domain" |
...
| No landing page beyond the README | Deliberate for now, and the reason is not the domain: the page is writable today and GitHub Pages would serve it without one. It is that a second surface carrying the accuracy, energy and terms claims is a second place for them to drift out of true, which is the exact defect this round found in the README's Liquid Glass sentence. The argument the plan asks to publish is on the README's first screen and links [accuracy.md](accuracy.md). Build the page when there is something the README cannot do — screenshots above the fold for a Product Hunt visitor — and generate its claims from the same source rather than restating them. Gated behind the public repository either way: Pages cannot serve a private one without a paid plan | README; [accuracy.md](accuracy.md); this file, "Domain" |
...
**Days 1–7, shippable at all.** MIT licence: done. Public repository: done, 2026-09-05. README GIF: done (`docs/media/demo.gif`, rendered from fixtures). `scripts/test.sh` running in automation: done, on every pull request that touches code (one that only changes markdown, `docs/` or `packaging/` skips the runner), on manual runs, and on every tag. Colour accessibility and `monospacedDigit`: done. Swift 6 `Sendable` warnings: resolved, and zero warnings under `Sources/` is a gate on every tag.
...
4. Show HN ([launch/show-hn.md](launch/show-hn.md), kept in the private internal repository). Gate: the signed DMG on Releases, and the tag's own release.yml run green (it has not had one yet).
