...
| `currency` | the rate the costs are converted at, or where it came from, changed: the code, the typed rate or *Fetch today's rate* changed, the ECB's rates arrived, or a held rate aged past its week | `code`, `rate`, `source` (`dollars`, `typed`, `reference`, `fallback`), `reason` on a fallback (`notYet`, `unreachable`, `unpublished`, `tooOld`), `day` (the ECB's day, on `reference` and on a `tooOld` fallback) |
