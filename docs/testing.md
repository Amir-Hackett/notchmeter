...
| `snapshot` | the distributed notification `com.amirhackett.notchmeter.oracle.snapshot` was received | every preference, `visibleTools`, `costCard` (`carried`, `leads`, `gaps`), `presence`, `sessions`, `sessionsCard` (`shown`, which is true for the empty card too; `rows`, each `id`, `group` (null when the card does not group), `status`, `source` (`hook`; `storage` for a row read from OpenCode's database; `detected` or `statusline` for one found without the hook; `coworkLog` for a Claude Cowork task), `agents`, `context` (0…1 or null), `todos` (`done`, `total`, or null), and since 0.11 `compaction` (`compacting`, `compacted` or null), `model` (the name the row shows, or null), `fellBack`, `switches`, `teammatesIdle`, `denials`, `worktree` and `note` (`waitingForAnswer`, `doneJump`, `justFinished`, `mcpInput`, `mayBeStuck`, or null), in drawn order, never a teammate's or an MCP server's name; `more`; `upgrade`, the assistant the card's *Install the hook…* line is for, or null when it is not drawn), `awaitingInput`, `signals` (each tool asking something of you, `<tool>:<state>`), `readings`, `advice`, `panelState`, `panelVisible`, `panelScreen`, `panelScroll` (`offset`, `insetTop`, `contentHeight`, `visibleHeight`, `scrollable`, `contentTopOnScreen`, `titleTopOnScreen`, `notchBottom`, `clearsNotch`; null while the panel is closed, and the notch fields null on the layouts with nothing over them), `regions`, `screens`, `captured`, `settingsVisible`, `settingsFrame`, `dashboardVisible`, `ringWindows` (each tool with a reading: the window ids its rings draw, outermost first), `agents` (per tool, each assistant's page switches as they take effect with the app-wide ones applied: `sessions`, `answers` (false where its hook has nothing to answer), `limitNotices`, `sessionNotices`) |
