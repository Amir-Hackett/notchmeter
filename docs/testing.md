...
| `pref` | any other preference changed | `key`, `value` (for `monthlyBudget` and `weeklyBudget`, `{amount, code}` as typed, or null when cleared) |
| `currency` | the rate the costs are converted at, or where it came from, changed: the code, the typed rate or *Fetch today's rate* changed, the ECB's rates arrived, or a held rate aged past its week | `code`, `rate`, `source` (`dollars`, `typed`, `reference`, `stale` for an ECB rate over a week old kept because no rate of the user's own is set, `fallback`), `reason` on a fallback (`notYet`, `unreachable`, `unpublished`, `tooOld`), `day` (the ECB's day, on `reference`, on `stale` and on a `tooOld` fallback) |
| `rate` | a request for the ECB's reference rates came back (only ever while *Fetch today's rate* is on and the code is not USD) | `action` (`fetched` / `failed`), `day` and `currencies` (how many the file held) when fetched, `failures` (in a row) when failed |
...
The snapshot also carries `currency`: the fields of the `currency` event, as they stand when it is asked for.
