...
| One assistant's page switched off, the others left on: Codex's *Read its sessions* off with a Codex and a Claude Code session running; then Codex's own *Answer from the notch* off with its sessions read again and a Codex approval raised; then Cursor's *Notify about its limits* off with a Cursor window behind pace | the Codex row leaves the Sessions card at the switch and does not come back on its next event, while its ring still refreshes on it; the Codex approval is asked in the Codex terminal and the panel shows only the wait, while a Claude Code request still opens its card; no Cursor pace banner, while another assistant's still arrives, and switching Cursor's back on reports the Cursor window at its next reading | oracle `pref key=sessionReadingOffTools`, `session action=forgotten tool=codex`, then `hook tool=codex sessions=off` with no `request`; `pref key=answerFromNotchOffTools` then `hook tool=codex wait=permission` with no `request` and no `panel cause=notification`; `pref key=limitNoticesOffTools` and no `notification action=scheduled` for Cursor; snapshot `agents` |
