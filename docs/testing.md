...
| `feedback` | the Send Feedback sheet came up, or its text was copied, sent or the sheet cancelled | `action` (`shown` / `copied` / `sent` / `cancelled`), `destination` (`github` / `email`), `diagnostics` (whether the report is included); on `copied` and `sent` also `route` (`browser`, `mailCompose`, `mailto`), `length` (characters of the body), `cut` (`none`, `logLines`, `diagnostics`, `message`), `omitted` (log lines left out, with `logLines`) and `replaced` (`projects`, `branches`, `titles`, `other`, `home`: how many different names of each kind the preview replaced); never a word of the message or the report, nor any name |
