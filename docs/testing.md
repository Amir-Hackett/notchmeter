...
| `notification` | a pace or advice alert was decided on, delivered, held by the ten-minute ceiling on a session's repeated waiting banner, clicked, or withdrawn from Notification Center (a window reset or moved back to on track, its tool was switched off or is no longer installed, or a session's wait ended) | `action` (`scheduled` / `sent` / `capped` / `clicked` / `removed`), `title`, `stage` when scheduled (`advice` for an advice notification), `level` when sent, `tool` and `identifier` when clicked or removed, `identifier` and `sinceLast` (seconds since the banner that spent this session's allowance) when capped, and `sound` when sent with a sound, naming the class it was chosen under (`pace`, `permission`, `question`, `plan`, `finished`; absent on a silent banner) |
| `hook` | a hook event arrived from Claude Code, Codex, Cursor, Gemini CLI, Copilot CLI, Kimi Code or OpenCode's plugin, or was read from OpenCode's database while the plugin is silent | `name` (Claude Code's event vocabulary for all seven; the other assistants' names are mapped onto it, so Gemini's `AfterAgent` and Copilot's `agentStop` both log as `Stop`), `needsInput`, `session`, `project`, `tool` only when the event is not Claude Code's (`codex`, `cursor`, `gemini`, `copilot`, `kimi`, `opencode`), `source` (`storage`) only for an event the app read from OpenCode's database rather than one a hook sent, `request` (`permission` or `question`) only when the hook is holding the session for an answer, `wait` (`permission`, `question` or `plan`: which waiting sound it would start) on any event that needs input or carries a request, settled before a request the notch will not answer is dropped, `todos` (`done`, `total`) on a `TodoWrite`'s `PostToolUse`, and `task` (`kind`, `status`) on a `TaskCreate`'s or `TaskUpdate`'s; never the title, the task list's words or a task's id, the tool summary, a question or a terminal id (docs/hooks.md, *What the hook sends*). An event from an assistant whose page has *Read its sessions* off carries `name`, `tool` and `sessions: "off"` and nothing else, since nothing else of it is kept |
| `decision` | the app answered a request, or handed it back to the terminal (the hold ran out, or *Answer in the terminal*) | `request` (the id), `kind` (`permission` or `question`), `behavior` (`allow`, `deny`, `answers`, `pass`), `session`, and on an allow `ruleRequested` (`true` when *Allow always* asked the hook for one of Claude Code's suggested rules, `false` for a plain Allow; the app cannot see whether the hook passed it on, so check the settings file for the rule itself); never what was asked or answered, nor which rule |
...
| `peek` | the collapsed strip began or stopped naming a session's news (NotchNews): shown when a wait begins or a long turn ends, hidden after four seconds (five under Reduce Motion), on a click, or when the panel opens | `action` (`shown` / `hidden`), `reason` (`approval`, `question`, `waiting`, `finished`), `session`, `tool`, and `source` (`coworkLog`) on a Claude Cowork task's finish, which no hook sent; never the project |
...
| `snapshot` | the distributed notification `com.amirhackett.notchmeter.oracle.snapshot` was received | every preference, `visibleTools`, `costCard` (`carried`, `leads`, `gaps`), `presence`, `sessions`, `sessionsCard` (`shown`, which is true for the empty card too; `rows`, each `id`, `group` (null when the card does not group), `status`, `source` (`hook`; `storage` for a row read from OpenCode's database; `detected` or `statusline` for one found without the hook; `coworkLog` for a Claude Cowork task), `agents`, `context` (0…1 or null), `todos` (`done`, `total`, or null), in drawn order; `more`; `upgrade`, the assistant the card's *Install the hook…* line is for, or null when it is not drawn), `awaitingInput`, `signals` (each tool asking something of you, `<tool>:<state>`), `readings`, `advice`, `panelState`, `panelVisible`, `panelScreen`, `panelScroll` (`offset`, `insetTop`, `contentHeight`, `visibleHeight`, `scrollable`, `contentTopOnScreen`, `titleTopOnScreen`, `notchBottom`, `clearsNotch`; null while the panel is closed, and the notch fields null on the layouts with nothing over them), `regions`, `screens`, `captured`, `settingsVisible`, `settingsFrame`, `dashboardVisible`, `ringWindows` (each tool with a reading: the window ids its rings draw, outermost first), `agents` (per tool, each assistant's page switches as they take effect with the app-wide ones applied: `sessions`, `answers` (false where its hook has nothing to answer), `limitNotices`, `sessionNotices`) |
