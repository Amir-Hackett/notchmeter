...
| `snapshot` | the distributed notification `com.amirhackett.notchmeter.oracle.snapshot` was received | every preference, `visibleTools`, `costCard` (`carried`, `leads`, `gaps`), `presence`, `sessions`, `sessionsCard` (`shown`, which is true for the empty card too; `rows`, each `id`, `group` (null when the card does not group), `status`, `source` (`hook`, or `coworkLog` for a Claude Cowork task), `agents`, `context` (0…1 or null), `todos` (`done`, `total`, or null), in drawn order; `more`), `awaitingInput`, `signals` (each tool asking something of you, `<tool>:<state>`), `readings`, `advice`, `panelState`, `panelVisible`, `panelScreen`, `panelScroll` (`offset`, `insetTop`, `contentHeight`, `visibleHeight`, `scrollable`, `contentTopOnScreen`, `titleTopOnScreen`, `notchBottom`, `clearsNotch`; null while the panel is closed, and the notch fields null on the layouts with nothing over them), `regions`, `screens`, `captured`, `settingsVisible`, `settingsFrame`, `dashboardVisible`, `ringWindows` (each tool with a reading: the window ids its rings draw, outermost first) |
