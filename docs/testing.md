...
| `snapshot` | the distributed notification `com.amirhackett.notchmeter.oracle.snapshot` was received | every preference, `visibleTools`, `costCard` (`carried`, `leads`, `gaps`), `presence`, `sessions`, `sessionsCard` (`shown`, which is true for the empty card too; `rows`, each `id`, `group` (null when the card does not group), `status`, `agents`, `context` (0…1 or null), `todos` (`done`, `total`, or null), in drawn order; `more`), `awaitingInput`, `signals` (each tool asking something of you, `<tool>:<state>`), `readings`, `advice`, `panelState`, `panelVisible`, `panelScreen`, `panelScroll` (`offset`, `insetTop`, `contentHeight`, `visibleHeight`, `scrollable`, `contentTopOnScreen`, `titleTopOnScreen`, `notchBottom`, `clearsNotch`; null while the panel is closed, and the notch fields null on the layouts with nothing over them), `regions`, `screens`, `captured`, `settingsVisible`, `settingsFrame`, `dashboardVisible`, `ringWindows` (each tool with a reading: the window ids its rings draw, outermost first), the Theme choices as stored (`panelTheme`, `panelMaterial` — null until one is chosen — `panelAccent`, `usageStyle`, `hourClock`) and `look`, what the panel is actually drawn in after the layout's default material and the accessibility settings that force it solid (`theme`, `material`, `accent`, `usageStyle`, `hourClock`, `contrast`) |
