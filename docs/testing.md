...
| `hook` | a hook event arrived from Claude Code, Codex, Cursor, Gemini CLI or Copilot CLI | `name` (Claude Code's event vocabulary for all five; the other assistants' names are mapped onto it, so Gemini's `AfterAgent` and Copilot's `agentStop` both log as `Stop`), `needsInput`, `session`, `project`, `tool` only when the event is not Claude Code's (`codex`, `cursor`, `antigravity`, `copilot`), and `request` (`permission` or `question`) only when the hook is holding the session for an answer; never the title, the tool summary, a question or a terminal id (docs/hooks.md, *What the hook sends*) |
