...
- whether another app is judged full-screen on the panel's display, with the display's size, the camera housing's band, the menu bar's current height and the windows that were weighed (`full screen:`), so a wrong verdict can be read off one line;
