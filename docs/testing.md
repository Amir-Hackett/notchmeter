...
| `pricing` | the price catalog (PricingCatalog) was applied from the cache at launch or when its switch was turned on, fetched, confirmed unchanged, refused, unreachable, or taken out of the tables by the switch | `action` (`applied` / `unchanged` / `refused` / `failed` / `off`), `from` (`cache`, `switch`, `network`), and on `applied` its `published` day, `entries` and `changed` (whether a rate any lookup answers with moved, which re-runs the cost scan); `why` and `failures` (in a row) on a refusal or failure; never the body |
| `snapshot` | the distributed notification `com.amirhackett.notchmeter.oracle.snapshot` was received | every preference, `visibleTools`, `costCard` (`carried`, `leads`, `gaps`, `range`: the range the card shows, and `prices`: the `PriceSource` keys behind the carried figures in that range, `builtIn:2026-09-24`, `catalog:2026-10-01`, `overrides`, `claudeCode`, `family`), `pricing` (`enabled`, `published`, `entries`, `confirmedAt`, `outcome`: the catalog in force and the last request's outcome), `presence`, `sessions`, `sessionsCard` (`shown`, which is true for the empty card too; `rows`, each `id`, `group` (null when the card does not group), `status`, `agents`, `context` (0…1 or null), `todos` (`done`, `total`, or null), in drawn order; `more`), `awaitingInput`, `signals` (each tool asking something of you, `<tool>:<state>`), `readings`, `advice`, `panelState`, `panelVisible`, `panelScreen`, `panelScroll` (`offset`, `insetTop`, `contentHeight`, `visibleHeight`, `scrollable`, `contentTopOnScreen`, `titleTopOnScreen`, `notchBottom`, `clearsNotch`; null while the panel is closed, and the notch fields null on the layouts with nothing over them), `regions`, `screens`, `captured`, `settingsVisible`, `settingsFrame`, `dashboardVisible`, `ringWindows` (each tool with a reading: the window ids its rings draw, outermost first) |
