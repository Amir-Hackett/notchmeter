...
| `decision` | the app answered a request, or handed it back to the terminal (the hold ran out, or *Answer in the terminal*) | `request` (the id), `kind` (`permission` or `question`), `behavior` (`allow`, `deny`, `answers`, `pass`), `session`; never what was asked or answered |
