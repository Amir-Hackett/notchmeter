...
| `clipboard` | a card or the panel was copied as an image, or text was copied (Copy diagnostics, Copy crash report, Send Feedback's Copy) | `kind`, and `width`, `height` for an image or `length` (characters) for text |
