...
**The scan for sessions without the hook** (0.9.0; [docs/hooks.md](hooks.md#sessions-found-without-the-hook)) is the one thing that runs more often than the minute tick, and it is sized to that. It runs every 3 seconds while an assistant is running and every 15 while none is, twice as long on battery or in Low Power Mode, and never while the screen is locked, the displays or the Mac are asleep, or another user is in front. One scan lists this user's processes (`proc_listpids` and one `proc_pidinfo` each), reads the arguments of the few named like an assistant or an interpreter, and `stat`s Claude Code's session files and a running session's transcript; the transcript's last 256 KB is read only once it has changed, and at most every ten seconds. Measured on 2026-09-24 on the same M5 Pro with one Claude Code session running: 10 ms for the first scan, which resolves each session's terminal once, and 2 ms for each one after, so about 0.07 % of one core at the 3-second pace and a sixth of that at the 15-second one. A scan that finds nothing new changes nothing the panel draws from, so it redraws nothing: a working row's clock is the turn's start and moves on its own timeline, and its `lastEvent` is written at most once a minute. *Find sessions without the hook* off stops it altogether.
