...
**The Claude Cowork watch** (0.9.0, not part of the measurement above) runs only while the Claude app is running and *Show Claude Cowork tasks* is on. Each pass is a directory listing per organisation folder and a `stat` or two per task, plus, for a task written to in the last thirty minutes, the bytes its log gained since the last pass (a task first seen reads the last 256 KB of its log, and once up to 8 MB when those settle nothing) and its half-megabyte record at most every thirty seconds while it changes. Passes are five seconds apart on mains power, ten on battery or in Low Power Mode, fifteen while no task is listed, and thirty while the screen is locked or the displays sleep; the Claude app quitting stops them altogether. It has not been measured on its own yet: the reproduction below with the Claude app open and a Cowork task running is the way to do it.
