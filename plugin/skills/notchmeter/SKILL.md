---
name: notchmeter
description: Read this Mac's AI usage windows (Claude Code session/weekly/per-model, Codex, Cursor, Gemini CLI, Antigravity, Copilot, Kimi Code, OpenCode), the local cost estimate and Notchmeter's advice before long work, so Claude can switch models or wait for a reset on its own. Use when the user asks how much quota is left, whether a long task fits before a reset, or when a task will run for more than a few minutes.
---
...
- A window whose `source` is `computedLocally` (OpenCode Go's 5-hour, weekly and monthly limits) was worked out on this Mac from its own turns at the prices and limits the vendor publishes, because the vendor offers no reading of its own; it counts nothing done on another machine, so say so when you cite it.
- `cost`: the Claude Code estimate at API list prices (`today`, `yesterday`, `week` since the weekly window started, `month`, `last30Days`, `last90Days`, `lastHour`, `burnMultiple`, the current `block`, `ranges` with per-model and per-project splits, and `priceSources`, the price tables behind the 30-day figure as `builtIn:2026-09-24`, `catalog:2026-10-01`, `overrides`, `claudeCode` or `family`, with each range's own under `ranges`). It is not a bill; on a subscription it is the API-equivalent value of the work.
